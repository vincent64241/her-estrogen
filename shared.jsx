// shared.jsx — shared data, components, and page sections for the HerEstrogen LP system.
// All CTA-bearing sections accept a `ctaUrl` prop so each page wires its own intake/quiz URL.
// Built into each page bundle via esbuild --bundle (no runtime module loader needed).
...
const TREATMENTS = [
  {
    ...
    mostPopular: true,
    features: [['Application', 'Twice weekly — adhesive patch']],
  ...
  },
  {
    ...
    features: [['Application', 'Once daily — inner arm or thigh']],
  ...
  },
  {
    ...
    features: [['Application', 'Once daily — oral tablet']],
  ...
  },
  {
    ...
    features: [['Application', 'Vaginal cream — as directed'], ['Form', 'Estrace Vaginal Cream']],
  ...
  },
  {
    ...
    features: [['Application', 'Nightly oral pill — at bedtime'], ['Dose', '100 mg or 200 mg as prescribed']],
...
// ─────────────────────────────────────────────────────────────────────────
// TIMELINE (How it Works animated)
// ─────────────────────────────────────────────────────────────────────────
      ...
      <div className="container">
        <div className="how-head">
          ...
          <p className="how-intro">
            A streamlined process built around your time. We've taken the three things that usually take six months — and made them take a week. <strong>You're supported every step.</strong>
          </p>
        </div>
            ...
            {TIMELINE_STEPS.map((s, i) => {
              ...
              return (
                <div className={'step-row ' + side} key={i}>
                  <div ref={(el) => cardRefs.current[i] = el} className={'step-card' + (visible[i] ? ' in' : '')}>
...
// ─────────────────────────────────────────────────────────────────────────
// MINI QUIZ CLOSING — accepts ctaUrl prop
// ─────────────────────────────────────────────────────────────────────────

function MiniQuizClosing({ ctaUrl }) {
  const SYMPTOMS_MINI = [
  ...
  function go() {
    ...
    window.location.href = count > 0 ? (url + sep + 'symptoms=' + encodeURIComponent(ids.join(','))) : url;
  }
  return (
...
function ProductsSection({ ctaUrl }) {
  return (
    <section className="treatments" id="treatments" data-screen-label="02 Treatments">
        ...
        <div className="treatments-two-col">
          ...
          <div className="treatments-col-right">
            <p className="treatments-sub">
              ...
              protocols. Your clinician decides what's appropriate based on your symptoms, history, and goals.
            </p>
          </div>
        </div>
      </div>
      <div className="products-scroll-wrapper">
        ...
        <div className="products-scroll-container">
          {TREATMENTS.map((t) => (
            <a href={ctaUrl} className={'product-card-new' + (t.mostPopular ? ' product-card-popular' : '')} key={t.id}>
              {t.mostPopular && <div className="product-popular-badge">Most Popular</div>}
              <div className="product-card-image-wrapper">
                <img src={t.image} alt={t.name} className={t.id === 'patch' ? 'no-shadow' : ''} />
              </div>
      ...
      <div className="container">
        ...
        <div className="home-promise-grid">
          ...
          <div className="home-promise-card">
            <div className="home-promise-icon" aria-hidden="true">
              <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <path d="M16 3h5v5"/><path d="M21 3l-7 7"/><rect x="3" y="8" width="13" height="13" rx="2" ry="2"/>
              </svg>
            </div>
    ...
    <section className="edu-section" id="why" data-screen-label="01b Why HRT">
      <div className="container">
        <div className="edu-head">
          ...
          <p className="edu-sub">
            HRT — hormone replacement therapy — gently puts back the estrogen and progesterone your body slowly stops making during perimenopause and menopause. A licensed clinician decides whether it's the right fit, and we route an FDA-approved option when it is.
          </p>
        </div>
        ...
        <div className="edu-table-wrap">
          ...
          <div className="edu-table-toggle-wrap">
            ...
            >
              {tableExpanded ? 'Show less ↑' : `See ${HRT_COMPARISON.length - collapsedCount} more symptoms ↓`}
            </button>
            {!tableExpanded && (
              <a href={ctaUrl} className="edu-table-cta-black">I FEEL THESE, LET'S GET STARTED</a>
            )}
          </div>
          {tableExpanded && (
            <div className="edu-table-cta-wrap">
              <a href={ctaUrl} className="edu-table-cta-pink">I'M READY, LET'S GO</a>
            </div>
          )}
        </div>
      </div>
...
function ScienceSection({ ctaUrl }) {
  return (
...
function WeekTimelineSection({ ctaUrl }) {
  return (
    <section className="week-timeline" data-screen-label="06 Week by week">
      <div className="container">
        ...
        <div className="wt-cta-wrap">
          <a href={ctaUrl} className="wt-cta-btn">GET STARTED</a>
        </div>
      </div>
    </section>
  );
...
function HrtFlowBand({ ctaUrl }) {
  return (
    <div className="hrt-flow-band">
      <Timeline />
      <WeekTimelineSection ctaUrl={ctaUrl} />
      <ClinicalSection />
    </div>
  );
}
...
function StartQuizSection({ ctaUrl }) {
  return (
  ...
  return (
    <footer data-screen-label="10 Footer">
      <div className="container foot-simple">
        ...
        <div className="foot-tag">Modern hormone care, written for the women medicine forgot.</div>
      ...
      <div className="container foot-disclaimer">
        <strong>Important Safety Information.</strong> Hormone therapy is a prescription treatment with real risks — including cardiovascular events, breast cancer (in combination with progestins), and others — that vary by individual, product, dose, and duration. Use the lowest effective dose for the shortest duration consistent with treatment goals and patient risk. Treatment is subject to clinician determination and may not be appropriate for everyone. This page is for informational purposes only and does not constitute medical advice. Individual results vary. Report side effects to FDA MedWatch at <a href="https://www.fda.gov/safety/medwatch" target="_blank" rel="noopener">fda.gov/safety/medwatch</a>. See full Privacy Policy and HIPAA Notice for how we handle your information.
      </div>
    </footer>
  );
}
...
export {
  SYMPTOMS, TREATMENTS, MARQUEE_ITEMS, HRT_COMPARISON, FAQ_DATA, TIMELINE_STEPS,
  MarqueeIcon, Timeline, MiniQuizClosing,
  TrustMarquee, ProductsSection, BonusPromisesSection, EduSection,
  ScienceSection, WeekTimelineSection, ClinicalSection, HrtFlowBand,

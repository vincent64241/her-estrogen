 ...
 * properties (`current_plan_length`, `product`) so segments can use them.
 *
 ...
 * @param {string} order.product         — single product name, e.g. "Estradiol Gel"
 *                                          or "Complete Protocol" for the bundle (required)
 * @param {number} order.value           — order value in dollars (required)
 * @param {string} [order.currency]      — defaults to 'USD'
 */
async function firePlacedOrder({ email, plan_length, product, value, currency }) {
  return postEvent({
    ...
    // "Complete Protocol Customer" can filter by them.
    profileProperties: {
 ...
 * we keep `current_plan_length` and `product` intact so historical segments
 * still work. Klaviyo's churn segments use the event timestamp.
 *
 * @param {object} cancel
 * @param {string} cancel.email          — customer email (required)
 * @param {number} [cancel.plan_length]  — 3, 6, or 12 (whatever plan they were on)
 * @param {string} [cancel.product]      — product they were on at time of cancel
 * @param {string} [cancel.reason]       — free-form cancel reason if available
 */
async function fireCancelledSubscription({ email, plan_length, product, reason }) {
  return postEvent({
